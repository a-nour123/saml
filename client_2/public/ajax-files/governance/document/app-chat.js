...
function enterChat(editFormSelector) {
            ...
            $('.chats').append(
              `<div class="chat">
                    <div class="chat-avatar">
                        <span class="avatar box-shadow-1 cursor-pointer ` + (response.data.note.user_id == response.data.document.created_by ? 'bg-light-success' : 'bg-light-primary') + ` ">
  ...
  let attachDocumentSelector = '';
  if (editFormSelector == '#show_contModal')
    attachDocumentSelector = '#attach-doc2';
  else if (editFormSelector == '#edit_contModal')
    attachDocumentSelector = '#attach-doc';


  if ($(attachDocumentSelector).length && $(attachDocumentSelector).val()) {

    editForm.find('.chat-app-form [name="document_id"]').val(editForm.find('input[name="id"]').val());
    ...
    $.ajax({
      ...
      , success: function (response) {
        if (response.status) {
          ...
          <div class="chat-body">
              <div class="chat-content">
                  <p class="download-doc-note-file cursor-pointer"><u>${response.data.note.display_name}</u></p>
                  <p style="text-align: right"><small><b>${response.data.note.created_at}</b></small></p>
              </div>
          </div>
        </div>`
          ...
          $(attachDocumentSelector).val('').trigger('change');
          $('.user-chats').scrollTop($('.user-chats > .chats').height());
        } else {
...
// Downloadnote file start
// Download note file - fixed version
$(document).on('click', '.download-doc-note-file', function() {
    const noteId = $(this).data('id');
  const documentId = $(this).data('document-id');
    const url = URLs['downloadDocumentCommentFile'] + "/" + noteId;
...
function addMessageToChat(document) {
    ...
    if (/\S/.test(note.note)) {
      ...
      if (user_id != note.user_id) {
        ...
        if ($('.chat:last-child').length == 0) {
          $('.chats').append(
            `<div class="chat chat-left user${note.user_id}">
                  <div class="chat-avatar">
                      <span class="avatar box-shadow-1 cursor-pointer ` + (note.user_id == document.created_by ? 'bg-light-success' : 'bg-light-primary') + ` ">
        ...
        if ($('.chat:last-child').hasClass('chat-left') && $('.chat:last-child').hasClass(`user${note.user_id}`)) {
          $('.chat:last-child .chat-body').append(
            `<div class="chat-content">`
            + (note.type == 't' ? `<p>${note.note}</p>` : `<p class="download-doc-note-file cursor-pointer" data-id='${note.id}' data-document-id='${document.id}'><u>${note.note}</u></p>`) +
